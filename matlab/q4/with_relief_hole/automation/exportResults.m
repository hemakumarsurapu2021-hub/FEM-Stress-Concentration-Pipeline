function exportResults(results, resultsDir)
% =========================================================================
%   exportResults
%   Aggregate per-case results into Excel (.xlsx), CSV (.csv) and
...
%   Kt_ellipse, Kt_circle, ratio_circle_ellipse,
%   delta_sigma_max,
...
% =========================================================================
    ...
    ratio       = arrayfun(@(r) r.ratioCircleToEllipse,  results);
    dsigMax     = arrayfun(@(r) r.deltaSigmaMax,         results);
    ...
    T = table( lc(:), nodes(:), elem(:), elemEll(:), elemCir(:), ...
               ...
               Kt_ell(:), Kt_cir(:), ratio(:), ...
               dsigMax(:), absErr(:), relErr(:), ...
        'VariableNames', { ...
            'lc', 'nodes', 'elements', 'elems_on_ellipse', 'elems_on_circle', ...
            'sigma_max_ellipse', 'sigma_max_circle', ...
            'Kt_ellipse', 'Kt_circle', 'ratio_circle_ellipse', ...
            'delta_sigma_max', 'abs_error', 'rel_error_pct'});
    ...
    fid = fopen(txtPath, 'w');
    fprintf(fid, 'Q4 FEM convergence study — summary\n');
    fprintf(fid, '%s\n\n', repmat('=', 1, 80));
    fprintf(fid, '%6s %8s %8s %10s %10s %12s %12s %9s %9s %10s %10s %10s %10s\n', ...
        'lc','nodes','elems','el_ellipse','el_circle', ...
        'smax_ell','smax_cir','Kt_ell','Kt_cir','ratio','dSigMax','absErr','relErr%');
    for i = 1:n
        fprintf(fid, '%6.3g %8d %8d %10d %10d %12.5f %12.5f %9.4f %9.4f %10.4f %10.4f %10.4f %10.3f\n', ...
            lc(i), nodes(i), elem(i), elemEll(i), elemCir(i), ...
            sigmaMaxEll(i), sigmaMaxCir(i), ...
            Kt_ell(i), Kt_cir(i), ratio(i), ...
            dsigMax(i), absErr(i), relErr(i));
    end
    fclose(fid);
